  ...
  describe('.updateAll()', function() {
    ...
    describe('attributes', function() {
      ...
      before(function(done) {
        ...
        for(var i=0; i<10; i++) {
          users.push({first_name: 'updateAll_user' + i, last_name: 'update', type: 'updateAll'});
        }
      ...
      it('should update model attributes', function(done) {
        User.updateAll({ type: 'updateAll' }, { last_name: 'updated' }, function(err, users) {
          assert(!err);
      ...
      it('should return model instances', function(done) {
        User.updateAll({ type: 'updateAll' }, { last_name: 'updated again' }, function(err, users) {
          assert(!err);
          assert(users[0].id);
          assert(users[0].fullName() === 'updateAll_user0 updated again');
    ...
    describe('find updated records', function() {
      ...
      before(function(done) {
        ...
        for(var i=0; i<2; i++) {
          users.push({first_name: 'updateAll_find_user' + i, last_name: 'update', type: 'updateAllFind'});
        }
        ...
        User.createEach(users, function(err, users) {
          ...
          // Update the 2 users
          User.updateAll({ type: 'updateAllFind' }, { last_name: 'Updated Find' }, function(err) {
            if(err) return done(err);
            done();
          });
        });
      ...
      it('should allow the record to be found', function(done) {
        User.findAll({ type: 'updateAllFind' }, function(err, users) {
          assert(!err);
