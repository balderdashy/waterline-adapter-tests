...
module.exports = function bootstrap( options, cb ) {
  ...
  // Merge in default options for tests
  options = _.merge({
    adapters: {
      wl_tests: Adapter // (`Adapter` is a global in tests)
    },
  ...
  // Fire the `fixture` event for each collection to allow the
  // tests to clean up after themselves in the physical-layer
  // schema.  (TODO: refactor this)
  _(extendedCollections).each(function (collectionProto) {
    Events.emit('fixture', collectionProto);
  });
