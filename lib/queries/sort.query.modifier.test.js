...
describe('Query', function() {
  ...
  describe('SORT Query Modifier', function() {
    ...
    it('should sort records using binary notation for asc', function(done) {
      User.findAll({ where: { type: 'sort test' }, sort: { dob: 1 } }, function(err, users) {
        assert(!err);
    ...
    it('should sort records using binary notation desc', function(done) {
      User.findAll({ where: { type: 'sort test' }, sort: { dob: 0 } }, function(err, users) {
        assert(!err);
        assert(users.length === 10);
    ...
    it('should sort records using atring notation for asc', function(done) {
      User.findAll({ where: { type: 'sort test' }, sort: 'dob asc' }, function(err, users) {
        assert(!err);
        assert(users.length === 10);
        assert(users[0].first_name === 'sort_user0');
        done();
    ...
    it('should sort records using atring notation for desc', function(done) {
      User.findAll({ where: { type: 'sort test' }, sort: 'dob desc' }, function(err, users) {
        assert(!err);
        assert(users.length === 10);
        assert(users[0].first_name === 'sort_user9');
        done();
